...
router.get('/v5/', function (req, res) {
  req.session.data = {}
  res.redirect('/v5/start')
})


// Do you know your NHS number?
router.post('/v5/do-you-know-nhs-number', function (req, res) {
  ...
  if (know == "Yes"){
    if (nhsNumber != "") {
      ...
      res.render('v5/do-you-know-nhs-number', {error: 1, noNhsNumber: 1})
    }
  }
  else {
    if (know == "No"){
      ...
      res.redirect('/v5/what-is-your-name')
    } else {
      res.render('v5/do-you-know-nhs-number', {error: 1, noAnswer: 1})
    }
  }
    ...
    if (month == ""){
      ...
      if (year == ""){
        ...
        if (!(dateRegex.test(year))) {
          ...
          res.redirect('/v5/what-is-your-postcode')
        } else {
...
router.post('/v5/what-is-your-postcode', function (req, res) {

  let postcode = req.session.data['postcode']

  if (postcode == ""){
    res.render('v5/what-is-your-postcode', {error: 1, noPostcode: 1})
  } else {
    res.redirect('/v5/accessible-format')
  }
...
router.post('/v5/accessible-format', function (req, res) {

  let checked = req.body['format']

  if (typeof checked == "undefined"){
    res.render('v5/accessible-format', {error: 1, noFormat: 1})
  } else {
    if (checked == "Not requested"){
      res.redirect('/v5/another-language')
    } else {
...
router.post('/v5/another-language', function (req, res) {

  let anotherLanguage = req.session.data['another-language']

  if (typeof anotherLanguage == "undefined"){
    res.render('v5/another-language', {error: 1, noOtherLanguage: 1})
  } else {
    if (anotherLanguage == "Yes"){
      res.redirect('/v5/languages')
    }
    else {
...
router.post('/v5/languages', function (req, res) {

  let language = req.session.data['language']

  if ((typeof language == "undefined") || (language == "")){
    res.render('v5/languages', {error: 1, noLanguage: 1})
  } else {
    res.redirect('/v5/check-your-answers')
  }
